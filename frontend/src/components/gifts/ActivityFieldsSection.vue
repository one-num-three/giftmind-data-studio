<template>
  <section class="section" data-section="activity" aria-labelledby="activity-title">
    ...
    <div class="fields"><label :class="{ 'ai-highlight': highlighted('activityDetails.durationMinutesMin') }">时长（分钟，起）<input v-model.number="modelValue.durationMinutesMin" type="number" min="0" /></label><label :class="{ 'ai-highlight': highlighted('activityDetails.durationMinutesMax') }">时长（分钟，止）<input v-model.number="modelValue.durationMinutesMax" type="number" min="0" /></label></div>
    <div class="fields"><label :class="{ 'ai-highlight': highlighted('activityDetails.participantsMin') }">参与人数（起）<input v-model.number="modelValue.participantsMin" type="number" min="0" /></label><label :class="{ 'ai-highlight': highlighted('activityDetails.participantsMax') }">参与人数（止）<input v-model.number="modelValue.participantsMax" type="number" min="0" /></label></div>
