    ...
    <div class="ai-messages">
      ...
      <section v-for="run in runs" :key="run.id" class="suggestions">
        <div class="suggestions__heading">
          <strong>字段建议</strong>
          <button type="button" @click="applyHighConfidence(run)">采用高可信建议</button>
        </div>
        <article v-for="patch in run.patches" :key="patch.path" :class="['patch', `patch--${patch.status}`]">
          ...
          <div class="patch__actions">
            <button type="button" :disabled="patch.status === 'applied'" @click="applyPatch(run, patch)">填入</button>
...
const props = defineProps<{ draftId: string; giftId?: string | null; giftTypeCode: string; currentValues: unknown }>();
const emit = defineEmits<{ "apply-field": [patch: FieldPatch] }>();
...
.suggestions { display: grid; gap: 8px; margin: 10px 0 18px; }.suggestions__heading { display: flex; align-items: center; justify-content: space-between; }.suggestions__heading button { border: 0; color: var(--color-primary); background: transparent; font-weight: 700; }.patch { padding: 11px; border: 1px solid #e2dfd2; border-radius: 12px; background: white; }.patch--applied { border-color: #79a98e; background: #eef7f1; }.patch--ignored { opacity: .55; }.patch > div { display: flex; justify-content: space-between; gap: 8px; }.patch small { color: var(--color-ink-muted); }.patch p { margin: 7px 0; color: var(--color-ink); }.patch__actions { justify-content: flex-end !important; }.patch__actions button { padding: 5px 9px; border: 1px solid var(--color-border); border-radius: 7px; background: white; }
