<template>
  <section class="workbench" aria-labelledby="workbench-title">
    <div class="workbench__heading">
      <div><p>单条礼物录入</p><h1 id="workbench-title">{{ giftId ? '编辑礼物' : '新建礼物' }}</h1></div>
      <span v-if="store.saving">保存中…</span>
    </div>
    <section v-if="restoredDraft" class="restore" aria-live="polite">
      <strong>发现未完成的本地草稿</strong><span>它只保存在此浏览器，尚未提交。</span>
      <div><button data-action="restore-draft" type="button" @click="restore">恢复草稿</button><button type="button" @click="discardDraft">丢弃</button></div>
    </section>
    <div class="workbench__grid">
      ...
      <form class="workbench__form" @input="store.markDirty" @submit.prevent="saveAndContinue">
        ...
        <GiftTypeSelector :model-value="draft.giftTypeCode" @select="selectType" />
    ...
    <section v-if="pendingType" class="confirm" role="alertdialog" aria-modal="true" aria-labelledby="confirm-title">
      <div><h2 id="confirm-title">切换礼物类型？</h2><p>{{ discardMessage }}</p><div><button type="button" @click="pendingType = null">保留当前类型</button><button data-action="confirm-type-switch" type="button" @click="confirmType">确认切换</button></div></div>
    </section>
  </section>
</template>
...
import { createActivityDraft, createProductDraft, useWorkbenchStore } from "../stores/workbench";
...
const discardMessage = computed(() => draft.value.giftTypeCode === "product" ? "商品专属信息不会用于活动记录。确认后将清除材质、配送等商品字段。" : "活动专属信息不会用于商品记录。确认后将清除时长、人数等活动字段。");

useUnsavedChanges(toRef(store, "dirty"));

onMounted(async () => {
  if (props.giftId) await store.load(props.giftId);
  else store.startNew();
});

function hasTypeSpecificValues() {
  return draft.value.giftTypeCode === "product"
    ? Boolean(draft.value.productDetails.genericProductName || draft.value.productDetails.materials.length || draft.value.productDetails.shippingRequired)
    : Boolean(draft.value.activityDetails.activityCategory || draft.value.activityDetails.durationMinutesMin !== null || draft.value.activityDetails.durationMinutesMax !== null || draft.value.activityDetails.participantsMin !== null || draft.value.activityDetails.participantsMax !== null || draft.value.activityDetails.pricingUnit);
}
function selectType(type: GiftTypeCode) {
  if (type === draft.value.giftTypeCode) return;
  if (hasTypeSpecificValues()) { pendingType.value = type; return; }
  applyType(type);
}
function commonDraftValues() {
  const { giftTypeCode: _giftTypeCode, productDetails: _productDetails, activityDetails: _activityDetails, ...common } = draft.value;
  return common;
}
function applyType(type: GiftTypeCode) {
  const common = commonDraftValues();
  store.replaceDraft(type === "product"
    ? { ...common, giftTypeCode: "product", productDetails: createProductDraft().productDetails }
    : { ...common, giftTypeCode: "activity", activityDetails: createActivityDraft().activityDetails });
}
