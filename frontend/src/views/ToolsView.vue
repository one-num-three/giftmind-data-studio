<template>
  <section class="tools"><div class="heading"><div><p>数据维护</p><h1>数据工具</h1><span>把重复录入交给选择、AI 和批量工具。</span></div></div>
    ...
    <div class="grid"><section class="card"><h2>DeepSeek 预判</h2><p>输入一个礼物名称，先查看模型建议，再回到录入页确认。</p><input v-model="aiName" placeholder="例如：南京博物院文创书签" /><select v-model="aiType"><option value="product">商品</option><option value="activity">活动</option></select><button type="button" @click="runAI">开始预判</button><pre v-if="aiResult">{{ JSON.stringify(aiResult, null, 2) }}</pre></section>
...
import { createCustomField, downloadBlob, listCustomFields, suggestGift, uploadFile } from "../api/tools";
const notice = ref(""); const fields = ref<Awaited<ReturnType<typeof listCustomFields>>>([]); const aiName = ref(""); const aiType = ref("product"); const aiResult = ref<unknown>(null); const field = reactive({ machineKey: "", displayName: "", valueType: "text" });
